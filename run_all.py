...
def main(cfg: Config):
	...
	# Split the image_paths into three parts
	image_paths_1 = image_paths[:len(image_paths) // 3]
	image_paths_2 = image_paths[len(image_paths) // 3:2 * len(image_paths) // 3]
	image_paths_3 = image_paths[2 * len(image_paths) // 3:]
	...
	image_paths = parts[cfg.part]
	
	print(f"Running on part: {cfg.part}")
	
	for idx, image_path in tqdm(enumerate(image_paths), total=len(image_paths)):
		
		print(f"Running on image {idx} out of {len(image_paths)}")
